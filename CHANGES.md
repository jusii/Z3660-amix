...
This branch adds Amiga Unix (AMIX -- Commodore's port of UNIX System V Release 4.0 to the 68030) interoperability on top of beta 20, and is published only on `jusii/Z3660-amix` (never pushed upstream). It carries the 68030 PMMU emulation, the A3000 SCSI / native piscsi / ethernet glue used to boot and run AMIX, and the fixes below. See `docs/AMIX.md` for setup.

 * **030 PMMU: fixed intermittent "User BUS ERROR" / panics from AMIX system tools under load.** When a 68030 instruction bus-faults during demand paging, the emulator resumes it mid-flight from its 030 bus-error frame (m68k_do_rte_mmu030 -> the run loop's inner retry loop). But the instruction-start PC was only snapshotted at the OUTER run-loop top, never when the inner loop re-ran a continuation-resumed instruction. So if the resumed instruction faulted AGAIN -- e.g. a MOVEM register-save to -(SP) in a function prologue growing the user stack across the next not-yet-resident page -- the new bus-error frame was rebuilt from the stale outer-loop PC (which by then was the kernel's RTE trap-return epilogue). The following RTE then returned the user process to that kernel PC in USER mode, so the next instruction fetch landed in unmapped user space: "User BUS ERROR at PC, PC:PC FAULT:6", frequently followed by a panic. Tools such as cron and in.telnetd died intermittently under fork/exec load. Fixed by re-snapshotting the instruction-start PC at the inner-loop continuation point, so a re-fault frames the resumed instruction's own PC (no-op for the common same-PC sub-access continue). Reproduced on real hardware -- a telnet-connect storm crashed ~100% of in.telnetd at a constant wild PC -- and after the fix the storm + fork/exec + deep-recursion stack-growth stress all run clean. (m68k_run_mmu030 in z3660_emu/.../newcpu.cpp; host MMU harness stays 60/60.)

 * **030 PMMU: fixed a residual of the above -- the in-RTE retry-access rebuilding a mis-attributed bus-error frame.** m68k_do_rte_mmu030 resumes a faulted instruction by re-performing its last access INSIDE the RTE (the "retry faulted access" block). If that access faults again (a still-unmappable page), the run-loop CATCH rebuilds the bus-error frame from mmu030_insn_start_pc and regs.irc -- but both still hold THIS RTE instruction's own identity (the trap-return PC ~0x0800129C and opcode 0x4E73), because the inner-loop re-snapshot only runs after the handler returns. So a format-$B rebuild inherited the RTE's PC (a wild user PC, e.g. 0x5C000000) and a format-$A rebuild inherited regs.irc as its frame opcode (a user clr.b 0x4218 flipped to 0x4E73) -- again "User BUS ERROR" under sustained demand-paging load. Fixed by re-pointing mmu030_insn_start_pc = pc and regs.opcode = regs.irc = mmu030_opcode at the instruction being resumed, just before the retry-access. The original telnet-storm wild-PC (constant 0x5C000000) no longer reproduces and re-fault frames rebuild correctly attributed; host MMU harness extended to 64/64 with a two-fault-continuation regression test. (m68k_do_rte_mmu030 in z3660_emu/.../cpummu030.cpp.)

 * **Warm reboot: raised the A3000-Kickstart auto-click cap so AMIX `reboot` (init 6) reliably clears the boot-loader prompt.** A warm AMIX reboot triggers a full firmware hard_reboot, which re-runs from scratch and re-arms the auto-click that dismisses the A3000 Kickstart's "press a mouse button to continue" prompt (cpu_emulator.cpp). That auto-click was capped at 6 presses -- occasionally too few to clear the prompt on the post-reboot boot, wedging it at the loader. Raised the cap to 64; it stays self-limiting (the press counter re-zeroes the instant the mouse-poll spin ends, so it never leaks past the prompt). Validated over a 24h reboot soak: 13 warm reboots, all recovered clean.

 * **A3000 SCSI: AMIX now autoboots from SCSI id 6 alone -- dropped the id-6 -> drive-0 alias band-aid.** The WD33C93 emulation hard-aliased SCSI target 6 to PISCSI backend drive 0 (a3000_scsi.cpp drvnum_for_target), a workaround from when Amix.hdf was configured only on scsi0 (drive 0) while AMIX's kernel opens its root at target 6. Consequence: the disk had to physically sit on scsi0 for AMIX to see id 6 at all; with the disk on scsi6 alone (scsi0 disabled) the alias probed the now-empty drive 0 -> CSR_TIMEOUT -> the Kickstart 3.1 RDB boot scan found no device on id 6 and refused to autoboot -- even though a KS 3.1 RDB autoboot is ID-agnostic (it boots by RDB BootPri, not by SCSI id; "must boot from id 0" is a myth). Fixed by making drvnum_for_target prefer the real id-6 backend (devs[6], where "scsi6 1" already loads Amix.hdf), falling back to the legacy drive-0 alias only when no id-6 backend exists, so old scsi0-only installs don't regress. The probe result is cached once per boot (drvnum_for_target is on the hot READ/WRITE path) and reset in a3000_scsi_init. HW-verified: with "scsi0 -1 / scsi6 1", AMIX autoboots from id 6 alone to multiuser. (a3000_scsi.cpp.)
